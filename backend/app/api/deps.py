...
from app.database.attention_store import get_attention_store
...
_attention_repo = AttentionRepository(get_attention_store())

_chat_service = ChatService(llm=_llm, repo=_repo)
_triage_service = TriageService(llm=_llm, repo=_repo)
_atencion_service = AtencionService(triage_service=_triage_service, repository=_attention_repo)
